...
def run(colors):
    ...
    while True:
        ...
        coords = int(coords[0] + facing.real), int(coords[1] + facing.imag)
...
colors = {(0, 0): 1}
run(colors)
left = min(x[0] for x in colors)
right = max(x[0] for x in colors)
top = max(x[1] for x in colors)
bottom = min(x[1] for x in colors)
for y in range(top, bottom-1, -1):
    for x in range(left, right+1):
        print('#' if colors.get((x, y), 0) else ' ', end='')
    print()
